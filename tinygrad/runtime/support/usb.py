...
ASM24_USB_IDS = ((0xADD1, 0x0001), (0x3801, 0x0001), (0x2065, 0x2463))
STOCK_UT3G_USB_IDS = ((0x2065, 0x2463),)

def list_asm24_devices():
  return list(itertools.chain.from_iterable(USB3.list_devices(*usb_id) for usb_id in ASM24_USB_IDS))

def asm24_gpu_bus(usb_id:tuple[int, int]) -> int:
  return 2 if usb_id in STOCK_UT3G_USB_IDS else 4
...
class USB3:
  ...
  def __init__(self, dev:c.POINTER[libusb.struct_libusb_device], ep_data_in:int, ep_stat_in:int, ep_data_out:int, ep_cmd_out:int,
    ...
    self.usb_id = (_desc.idVendor, _desc.idProduct)
    ...
    if self.is_custom or self.usb_id in STOCK_UT3G_USB_IDS: self.use_bot = use_bot = True
    ...
    if checked(libusb.libusb_kernel_driver_active)(self.handle, 0):
      ...
      # Resetting a stock BOT device after detach can wedge the comma a600000 xHCI controller.
      # The custom firmware still requires the historical reset during interface takeover.
      if self.usb_id not in STOCK_UT3G_USB_IDS: checked(libusb.libusb_reset_device)(self.handle)
